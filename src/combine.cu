   ...
   */
    ...
    for (int i = 0; i < (n+TILE-1)/TILE; ++i) {
      ...
      for (int k = 0; k < TILE; ++k) {
        accumlate_value += a_shared[local_row][k] * b_shared[k][local_col];
      }
      __syncthreads();
    }
...
__global__ void mapKernel(
   ...
   *  out_shape: shape of the output array, out_shape could be longer than in_shape, but broadcastable to in_shape
   *  out_strides: strides of the output array
   ...
   */
    ...
    if (i < out_size) {
      ...
      for (int j = 0; j < a_shape[reduce_dim]; ++j) {
        ...
        out[i] = fn(fn_id, out[i], a_storage[in_pos]);
      }
    }
    /// END ASSIGN1_2
}
